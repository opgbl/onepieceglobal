...
async function getEpisode(id){
 const tries=[`/api/episode?id=${encodeURIComponent(id)}`,`/episode?id=${encodeURIComponent(id)}`,`/episodes/${encodeURIComponent(id)}`,`/v1/episode?id=${encodeURIComponent(id)}`,`/api/v1/episode?id=${encodeURIComponent(id)}`];
 for(const t of tries){
  try{
   let r=await apiFetch(t,{method:"GET"});
   if(r.ok){const j=await r.json();if(j&&typeof j==="object")return j}
   if(r.status===401||r.status===405){
    const tk=window.__tsGate&&window.__tsGate.get?window.__tsGate.get():null;
    r=await apiFetch(t,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({token:tk,id})});
    if(r.ok){const j=await r.json();if(j&&typeof j==="object")return j}
   }
  }catch(e){}
 }
